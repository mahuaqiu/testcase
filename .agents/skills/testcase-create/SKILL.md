...
生成的测试用例不直接调用 `TestagentClient`，也不自行查询、取消或重试 Worker 任务。异步任务的幂等提交、状态轮询、取消和结构化错误由公共基础设施处理，测试代码只通过 `User` 调用 AW 方法。

`parallel()` 只包裹互不依赖的操作；登录、入会等操作完成后，在并行块外顺序执行断言。不要在用例中假设任务查询只返回一次，也不要根据 Worker 错误字符串编写分支。
