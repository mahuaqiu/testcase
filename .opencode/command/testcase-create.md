---
description: "自动化测试用例生成。直接执行全流程，生成前用户评审，生成后复查验证。"
---

# 自动化测试用例生成

直接执行测试用例生成全流程。需要AW操作时调用 /testcase-aw 命令。
...
发现需要新增/扩展AW时，调用 /testcase-aw 命令。
...
4. **需要AW时调用 /testcase-aw**：AW操作专用命令
